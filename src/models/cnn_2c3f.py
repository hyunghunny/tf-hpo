...
class CNN2C3F(ModelInterface):
    ...
    def initialize(self):
        with tf.device(self.flag["TRAIN_DEVICE_ID"]):    
            ...
            resized = int(round(self.data["IMAGE_SIZE"] / self.hp["STRIDE_SIZE"] / self.hp["STRIDE_SIZE"] ))
            self.fc1_weights = tf.Variable(    # fully connected
                    tf.truncated_normal(                          
                            [resized * resized * self.hp["CONV2_DEPTH"], 
                             self.hp["FC1_WIDTH"]],
                            stddev=self.hp["INIT_STDDEV"],
                            seed=self.hp["SEED"]))
